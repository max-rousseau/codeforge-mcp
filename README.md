...
**Timeouts terminate sandbox processes**: The `timeout` argument on `execute_code` is enforced inside the sandbox container by wrapping the `deno` invocation with coreutils `timeout(1)` (SIGTERM at the budget, SIGKILL 2s later as a fallback). The tool call rejects with an "Execution timed out" error and the underlying Deno process is signalled — no orphan processes accumulate. The sandbox container itself is unaffected; only the per-exec Deno process is killed.
